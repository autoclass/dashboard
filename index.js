...
function listItems(items) {
    return items?.map(i => `<li class="list-group-item text-muted text-monospace">
                                ${i}
                                <button type="button" class="close stretched-link" aria-label="Delete" data-id="${i}" onclick="(function n(){v = new Event('playSound'); v.whom=this; document.dispatchEvent(v)}).call(this)">
...
$(
    () => {
        $(document).on("resetAhk", () => {
            ...
            addToLog(`Reset ${target}'s AHK script`);
        });
    }
)
